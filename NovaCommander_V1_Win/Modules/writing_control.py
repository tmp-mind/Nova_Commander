...
def insert_at_symbol(command_text=None):
    keyboard.press_and_release('ctrl+alt+à')  # Utilise `ctrl` + `alt` + `à` pour @
...
# Fonction pour insérer les parenthèse
def insert_parenthesis(command_text=None):
    keyboard.write('(')
    keyboard.write(')')
    keyboard.press_and_release('left')  # Place le curseur entre les parenthèses
...
def insert_braces(command_text=None):
    keyboard.write('{')
    keyboard.write('}')
    keyboard.press_and_release('left')  # Place le curseur entre les accolades
...
def write_text(command_text):
    ...
    # Utilisation de `keyboard.write` pour gérer les caractères spéciaux correctement
    keyboard.write(final_text.strip())
    print(f"Texte écrit : {final_text.strip()}")
