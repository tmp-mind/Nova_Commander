...
import time
import pyautogui  # Utilisé pour les actions clavier et souris sous Windows
...
COMMANDS = {
    ...
    "active le module dofus": "start_dofus_module",
    "active le module de jeu": "start_game_module",
    "active le module de messagerie wakfu": "start_message_module",
...
def start_writing_mode(command_text=None):
    ...
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{writing_script}'"], shell=True)
...
def start_os_controller(command_text=None):
    ...
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{os_controller_script}'"], shell=True)
    
    speak("System control module activated.")

def start_wakfu_module(command_text=None):
    """Lance le script de contrôle du curseur pour le jeu Wakfu dans un nouveau terminal"""
    speak("Launching Wakfu control module.")
    
    # Définir le chemin du script de contrôle du curseur pour le jeu Wakfu
    wakfu_script = os.path.join(os.path.dirname(__file__), 'Modules', 'cursor_games_wakfu.py')
    
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{wakfu_script}'"], shell=True)
    
    speak("Wakfu control module activated.")

def start_dofus_module(command_text=None):
    """Lance le script de contrôle du curseur pour le jeu générique dans un nouveau terminal"""
    speak("Launching dofus control module.")
    
    # Définir le chemin du script de contrôle du curseur pour le jeu Wakfu
    dofus_script = os.path.join(os.path.dirname(__file__), 'Modules', 'cursor_games_dofus.py')
    
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{dofus_script}'"], shell=True)
    
    speak("Dofus control module activated.")

def start_game_module(command_text=None):
    """Lance le script de contrôle du curseur pour le jeu Wakfu dans un nouveau terminal"""
    speak("Launching games control module.")
    
    # Définir le chemin du script de contrôle du curseur pour le jeu Wakfu
    games_script = os.path.join(os.path.dirname(__file__), 'Modules', 'cursor_control_games.py')
    
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{games_script}'"], shell=True)
    
    speak("Generic Games control module activated.")

# Module de messagerie wakfu
def start_message_module(command_text=None):
    """Lance le script de contrôle du curseur pour le jeu Wakfu dans un nouveau terminal"""
    speak("Launching message wakfu module.")
    
    # Définir le chemin du script de contrôle du curseur pour le jeu Wakfu
    games_script = os.path.join(os.path.dirname(__file__), 'Modules', 'only_message_wakfu.py')
...
def start_cursor_move(command_text=None):
    """Lance le script de contrôle du curseur dans un nouveau terminal"""
    ...
    # Lancer le script dans un nouveau terminal PowerShell réduit
    subprocess.Popen(["start", "powershell", "-WindowStyle", "Minimized", "-Command", f"python '{cursor_script}'"], shell=True)
...
def open_youtube(command_text=None):
    """Ouvre YouTube dans Chrome et informe l'utilisateur"""
...
def target_word(command_text=None):
    ...
    if word_to_target:
        ...
        pyautogui.hotkey('ctrl', 'f')
        pyautogui.typewrite(word_to_target)
        pyautogui.press('enter')  # Valide la recherche
